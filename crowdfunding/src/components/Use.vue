...
import { getAccount, getAllUse, getProve,Funding, agreeUse,newProve, newUse, Use, addListener } from '../api/contract'
...
const columns = [
  ...
  },
  {
    dataIndex: 'goal',
    key: 'goal',
    title: '使用金额(UGAS/万人民币)'
  },
  {
    dataIndex: 'agreeAmount',
    key: 'agreeAmount',
    title: '同意请求数额(UGAS/万人民币)'
  },
  {
    dataIndex: 'disagree',
    key: 'disagree',
    title: '不同意请求数额(UGAS/万人民币)'
  },
  {
    dataIndex: 'over',
    key: 'over',
...
export default defineComponent({
  ...
  setup(props) {
    ...
    async function fetchData() {
      state.loading = true;
      try {
        // await newProve(props.id as number,"cscs","cssvdfvfv","bg");
        state.data = await getAllUse(props.id as number);
      //  console.log(await getProve(props.id as number),"4656");
